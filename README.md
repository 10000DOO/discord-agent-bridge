...
1. 채널에서 **`/agent start`** → **채널 만들기 마법사**가 순서대로 안내합니다:
   **작업 폴더 선택 → 백엔드(Claude / Codex) → 모델 → 권한 모드/프로필**
2. 만들어진 세션 채널에서 **그냥 메시지로 대화**하면 됩니다. Claude 모드는 스트리밍 출력·툴 실행 스레드·권한 승인 버튼이 뜹니다.
3. 필요할 때 명령어 사용.

### 주요 명령어
| 명령어 | 설명 |
|---|---|
| `/agent start` | 새 세션 시작 (폴더·백엔드·모델·권한 선택 마법사) |
| `/agent resume` | 이전 세션 이어하기 |
| `/agent close` | 세션 종료 (채널 정리) |
...
MCP·서브에이전트·스킬·훅 등은 각 백엔드의 **전역/프로젝트 설정을 자동으로 로드**해 그대로 동작합니다.
