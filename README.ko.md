...
디스코드 안의 `/update` 명령은 두 설치 방식 모두에서 동작하며, 거치는 경로만 다릅니다([자동 업데이트](#자동-업데이트) 참고). 이 방식은 `install.sh`를 실행하고 launchd 에이전트를 그 자리에서 재시작합니다.
...
`/update`가 릴리스 레지스트리를 확인하고, 승인하면 설치와 서비스 재시작까지 대신 해 줍니다. config의 `autoUpdate.enabled`로 끌 수 있습니다. 어느 경로를 타는지는 `dab`을 어떻게 설치했는지에 달렸고, 두 경로가 섞이는 일은 없습니다 — Homebrew 설치를 소스 경로로 몰면 엉뚱한 위치에 두 번 재시작하는 문제가 있었기 때문입니다.

| 설치 방식 | `/update`가 하는 일 |
|---|---|
| Homebrew | 탭의 `homebrew-self-update.sh`에 넘깁니다. `brew upgrade` → 서비스 정지·시작 → 정상 기동 확인 → 실패 시 이전 버전으로 롤백. 직접 `brew upgrade dab`을 쳐도 결과는 같습니다. |
| 소스 빌드 | 플랫폼 설치 경로를 실행하고 서비스를 그 자리에서 재시작합니다(macOS: `install.sh` + launchctl). 저장소 체크아웃이 있어야 합니다. |

Homebrew 설치인데 탭 스크립트가 없거나 실행되지 않으면, 소스 경로로 넘어가지 않고 그 사실을 알린 뒤 멈춥니다 — 이때는 탭을 다시 설치하세요. 어느 쪽이든 알림은 새 정식 버전당 한 번만 뜨고, 이미 설치한 버전을 다시 권하지 않습니다.

재시작하면 그 순간 진행 중이던 답변 하나는 유실되지만 **대화는 끊기지 않습니다.** 채널별 세션 식별자를 저장해 두었다가 다음 메시지에서 이어 열기 때문에, 업데이트해도 하던 이야기가 그대로 이어집니다.
