...
- **예외 없음 — 오타 하나·한 줄 변경처럼 아무리 사소한 수정이라도 `report` 호출 전에는 미완료다.** 채팅창에 결과만 적고 턴을 끝내는 것은 보고가 아니다.
- `report`가 **`busy`를 반환하면 잠시 후 재시도**한다(총괄 채널이 다른 턴을 처리 중이라는 뜻 — 즉시 다른 경로로 우회하지 않는다).
...
- [ ] 작업이 끝났다 → **`report` 도구를 호출했다** — 이 둘은 별개의 사건이다. 작업 규모(오타 수정 한 건이라도)는 예외 사유가 아니다.
